...
define([ 'dojo/string', '../Module', '../Value', '../console', '../node!fs' ],
	...
	function processReturns(/**Array*/ returns) {
		...
		for (var i = 0, returnValue; (returnValue = returns[i]); ++i) {
			if(returnValue.metadata.type || returnValue.type !== 'undefined'){
				returnTypes[returnValue.metadata.type || returnValue.type] = 1;
			}
		}

		for (var k in returnTypes) {
			var t = k.toLowerCase();
			if (returnTypes.hasOwnProperty(k) && types.indexOf(t) === -1) {
				types.push(t);
			}
		}
